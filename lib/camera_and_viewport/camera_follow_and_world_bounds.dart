...
class CameraFollowAndWorldBoundsExample extends FlameGame
  ...
  Future<void> onLoad() async {
    ...
    final player = Player(joystick)..position = Vector2(250, 0);
    camera
...
class Player extends PositionComponent with KeyboardHandler {
  ...
  void keyInput(JoystickComponent joystick) {
    // if (!joystick.delta.isZero()) {
    //   joystick.delta.normalize();
    //   velocity.x = joystick.delta.x;
    //   velocity.y = joystick.delta.y;
    ...
    joystick.delta.normalize();
    print("huze: joystick.delta is ${joystick.delta}");
    final isKeyDown = !joystick.delta.isZero();
    final keyLeft = joystick.delta.x < 0;
    final keyRight = joystick.delta.x > 0;
    final keyUp = joystick.delta.y < -0.9;
    ...
    if (isKeyDown) {
      ...
      if (keyUp && nJumpsLeft > 0) {
        velocity.y = velocity.y - jumpSpeed;
        nJumpsLeft -= 1;
      }
    } else {
      final hasLeft = joystick.delta.x < 0;
      final hasRight = joystick.delta.x > 0;
      if (hasLeft && hasRight) {
